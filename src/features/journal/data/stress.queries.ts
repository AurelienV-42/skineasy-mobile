...
import * as stressApi from '@features/journal/data/stress.api';
...
export function useStressEntries(date: string) {
  return useQuery({
    ...
    queryFn: () => stressApi.getStressByDate(date),
    staleTime: 5 * 60 * 1000,
  });
}
...
export function useUpsertStress() {
  ...
  return useMutation({
    mutationFn: (dto: CreateStressEntryDto) => stressApi.upsertStress(dto),
...
export function useDeleteStress() {
  ...
  return useMutation({
    mutationFn: ({ id }: { id: string; date: string }) => stressApi.deleteStress(id),
